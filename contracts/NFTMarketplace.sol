...
contract NFTMarketplace is ERC721URIStorage {
    ...
    /// basically that means that the owner of the marketplace will get 0.025 ETH for every NFT that is listed on the marketplace. 💸
    /// but since we're deploying to polygon, we'll use the MATIC token instead of ETH.
    /// 0.025 MATIC = 0.018743 USD as of 09/2022
    ...
    constructor() ERC721("Foxy Tokens", "FXT") {
    ...
    /// It's public so we can access it from the frontend (client side)
    ...
    /// this one is private, so we're not able to access it from the frontend (client side)
    /// but we can use it inside this very contract, as we're already doing in createToken()
    function createMarketItem(uint256 tokenId, uint256 price) private {
      // require a certain CONDITION, in this case price greater than 0
      require(price > 0, "Price must be at least 1 wei");
      // require that the users are sending in the transaction is sending in the correct amount
    ...
    /* Transfers ownership of the item (from marketplace to user that bought it), as well as funds between parties */
    ...
    /// view flag for the functions means, that it's not going to implement any logic, it's just going to return some information.
    function fetchMarketItems() public view returns (MarketItem[] memory) {
      ...
      // the type of the element in the array is marketitem, and the unsolditemcount is the length
    ...
    /* Returns only items a user has listed (has put for sale on the marketplace) */
